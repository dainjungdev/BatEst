function [MSMR_parameters, RMSE] = get_MSMR_params_five(OCP_filename)
generate_plot = true;
...
Q_initial = zeros(1, 5);
U0_initial = zeros(1, 5);
w_initial = zeros(1, 5);

dSOC_dV_to_fit = dSOC_dV;

% Define peak location bounds
peakLoc_bounds = [800 1500; 1200 1600; 1400 1800; 1700 2000; 0 1200];
...
for j = 1:5
    ...
    for i = 1:length(range)
        ...
        elseif ~isnan(V_ref_left)
            ...
            if counter >= max(length(range) - i) break; end
        ...
        elseif ~isnan(V_ref_right)
            ...
            counter = counter + 1;
            w_initial(j) = (w_initial(j) * (i - 1/2 - 1/2 * counter) + w * 1/2) / (i - 1/2 * counter);
            if counter >= min(length(range) - i) break; end
    ...
    % 3) compute Q_initial
    Q_initial(j) = dSOC_dV_peak * (-4) * w_initial(j) * (1 - 0.1*0.5^j)/ f; 
...
% MSMR_parameters = [MSMR_parameters(1,:); MSMR_parameters; MSMR_parameters(end,:)];
% MSMR_parameters(1,2) = MSMR_parameters(1,2)* 1/3; MSMR_parameters(2,2) = MSMR_parameters(2,2)* 2/3;
% MSMR_parameters(5,2) = MSMR_parameters(5,2)* 1/3; MSMR_parameters(6,2) = MSMR_parameters(6,2)* 2/3;
